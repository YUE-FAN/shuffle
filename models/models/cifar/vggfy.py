...
class DCONV_3x3(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, kernelsize, stride, padding, bias, type):
        ...
        self.dconv = Dconv_shuffle(inplanes, outplanes, kernelsize, stride, padding, bias)
        self.bn = nn.BatchNorm2d(outplanes)
        self.relu = nn.ReLU(inplace=True)
...
class VGG16(nn.Module):  # TODO: try different config of the channels
    def __init__(self, dropout_rate, num_classes, include_top, layer, type='none'):
        ...
        if layer > 11:
            ...
            self.conv11 = DCONV_3x3(3, 64, kernelsize=3, stride=1, padding=1, bias=self.bias, type=type)

        if layer > 12:
            self.conv12 = nn.Sequential(CONV_3x3(64, 64, kernelsize=3, stride=1, padding='same', bias=self.bias),
                                        nn.MaxPool2d(kernel_size=2, stride=2))
            # self.conv12 = CONV_3x3(64, 64, kernelsize=3, stride=2, padding='same', bias=self.bias)
        else:
            self.conv12 = DCONV_3x3(64, 64, kernelsize=3, stride=2, padding=1, bias=self.bias, type=type)

        if layer > 21:
            self.conv21 = CONV_3x3(64, 128, kernelsize=3, stride=1, padding='same', bias=self.bias)
        else:
            self.conv21 = DCONV_3x3(64, 128, kernelsize=3, stride=1, padding=1, bias=self.bias, type=type)

        if layer > 22:
            self.conv22 = nn.Sequential(CONV_3x3(128, 128, kernelsize=3, stride=1, padding='same', bias=self.bias),
                                        nn.MaxPool2d(kernel_size=2, stride=2))
            # self.conv22 = CONV_3x3(128, 128, kernelsize=3, stride=2, padding='same', bias=self.bias)
        else:
            self.conv22 = DCONV_3x3(128, 128, kernelsize=3, stride=2, padding=1, bias=self.bias, type=type)

        if layer > 31:
            self.conv31 = CONV_3x3(128, 256, kernelsize=3, stride=1, padding='same', bias=self.bias)
        else:
            self.conv31 = DCONV_3x3(128, 256, kernelsize=3, stride=1, padding=1, bias=self.bias, type=type)

        if layer > 32:
            self.conv32 = CONV_3x3(256, 256, kernelsize=3, stride=1, padding='same', bias=self.bias)
        else:
            self.conv32 = DCONV_3x3(256, 256, kernelsize=3, stride=1, padding=1, bias=self.bias, type=type)

        if layer > 33:
            self.conv33 = nn.Sequential(CONV_3x3(256, 256, kernelsize=3, stride=1, padding='same', bias=self.bias),
                                        nn.MaxPool2d(kernel_size=2, stride=2))
            # self.conv33 = CONV_3x3(256, 256, kernelsize=3, stride=2, padding='same', bias=self.bias)
        else:
            self.conv33 = DCONV_3x3(256, 256, kernelsize=3, stride=2, padding=1, bias=self.bias, type=type)

        if layer > 41:
            self.conv41 = CONV_3x3(256, 512, kernelsize=3, stride=1, padding='same', bias=self.bias)
        else:
            self.conv41 = DCONV_3x3(256, 512, kernelsize=3, stride=1, padding=1, bias=self.bias, type=type)

        if layer > 42:
            self.conv42 = CONV_3x3(512, 512, kernelsize=3, stride=1, padding='same', bias=self.bias)
        else:
            self.conv42 = DCONV_3x3(512, 512, kernelsize=3, stride=1, padding=1, bias=self.bias, type=type)

        if layer > 43:
            self.conv43 = nn.Sequential(CONV_3x3(512, 512, kernelsize=3, stride=1, padding='same', bias=self.bias),
                                        nn.MaxPool2d(kernel_size=2, stride=2))
            # self.conv43 = CONV_3x3(512, 512, kernelsize=3, stride=2, padding='same', bias=self.bias)
        else:
            self.conv43 = DCONV_3x3(512, 512, kernelsize=3, stride=2, padding=1, bias=self.bias, type=type)

        if layer > 51:
            self.conv51 = CONV_3x3(512, 512, kernelsize=3, stride=1, padding='same', bias=self.bias)
        else:
            self.conv51 = DCONV_3x3(512, 512, kernelsize=3, stride=1, padding=1, bias=self.bias, type=type)

        if layer > 52:
            self.conv52 = CONV_3x3(512, 512, kernelsize=3, stride=1, padding='same', bias=self.bias)
        else:
            self.conv52 = DCONV_3x3(512, 512, kernelsize=3, stride=1, padding=1, bias=self.bias, type=type)

        if layer > 53:
            self.conv53 = nn.Sequential(CONV_3x3(512, 512, kernelsize=3, stride=1, padding='same', bias=self.bias),
                                        nn.MaxPool2d(kernel_size=2, stride=2))
            # self.conv53 = CONV_3x3(512, 512, kernelsize=3, stride=2, padding='same', bias=self.bias)
        else:
            self.conv53 = DCONV_3x3(512, 512, kernelsize=3, stride=2, padding=1, bias=self.bias, type=type)
