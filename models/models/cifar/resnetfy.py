...
def conv_1_3x3_dconv():
    return nn.Sequential(Dconv_shuffle(3, 64, 3, 1, 1, bias=False),
                         # DConv1Dai(3),
...
class bottleneck_dconv(nn.Module):
    def __init__(self, inplanes, planes, kernel_size, strides=(2, 2), type='error'):
        ...
        self.dconv1 = Dconv_shuffle(plane1, plane2, kernel_size=kernel_size, stride=strides, padding=int((kernel_size - 1) / 2), bias=False)
        ...
        self.dconv2 = Dconv_shuffle(inplanes, plane3, kernel_size=1, stride=strides, padding=0, bias=False)
        self.bn4 = nn.BatchNorm2d(plane3)
        self.relu = nn.ReLU(inplace=True)
...
class identity_block3_dconv(nn.Module):
    def __init__(self, inplanes, planes, kernel_size, type='error'):
        ...
        # else:
        #     raise Exception('The type of the dconv does not exit')
        self.dconv = Dconv_shuffle(plane1, plane2, kernel_size=kernel_size, stride=1, padding=int((kernel_size - 1) / 2), bias=False)
        self.bn2 = nn.BatchNorm2d(plane2)
