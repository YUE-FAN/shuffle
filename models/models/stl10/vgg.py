...
class CONV_3x3_save(nn.Module):
    ...
    def forward(self, x):
        ...
        # np.save('/nethome/yuefan/fanyue/dconv/weight.npy', self.conv.weight.detach().cpu().numpy())
        x = self.conv(x)
        x = self.bn(x)
        x = self.relu(x)
        return x
...
class DCONV_3x3(nn.Module):
    ...
    def __init__(self, inplanes, outplanes):
        ...
        self.dconv = Dconv_none(inplanes, outplanes)
        self.bn = nn.BatchNorm2d(outplanes)
        self.relu = nn.ReLU(inplace=True)
...
class VGG16(nn.Module):  # TODO: try different config of the channels
    def __init__(self, dropout_rate, num_classes, include_top):
        ...
        self.avgpool = nn.AvgPool2d(3)  # TODO: check the final size
        self.fc = nn.Linear(512, num_classes)
...
class VGG16_Transpose(nn.Module):  # TODO: try different config of the channels
    def __init__(self, dropout_rate, num_classes, include_top):
        ...
        self.conv42 = CONV_3x3(512, 512, kernelsize=3, stride=1, padding='same', bias=False)
        # self.conv42 = DCONV_3x3(512, 512)
        self.conv43 = CONV_3x3(512, 144, kernelsize=3, stride=2, padding='same', bias=False)

        self.conv51 = CONV_3x3(36, 144, kernelsize=3, stride=2, padding='same', bias=False)
        self.conv52 = CONV_3x3(36, 144, kernelsize=3, stride=2, padding='same', bias=False)
        self.conv53 = CONV_3x3(36, 144, kernelsize=3, stride=2, padding='same', bias=False)
