"""
This script computes the difference between model acc and ref acc for each class.
...
Examples:
python classwise_analysis.py -d cifar100 -j 0 --dataset-path /BS/databases00/cifar-100 --manualSeed 6 --gpu-id 0
-a vgg16_1x1lmp --resume /BS/yfan/work/trained-models/dconv/checkpoints/cifar100/vgg161x1lmp_300_imgsize32_DA/
...
cifar10_id2label = ['airplane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']
imagenet_id2label = {0: 'tench, Tinca tinca',
...
def draw_save_table(acc_diff, layer_list, sort_index, id2label, save_path):
    ...
    axs.axis('tight')
    axs.axis('off')
    the_table = axs.table(cellText=clust_data, rowLabels=rowLabels, colLabels=collabel, loc='center')
    the_table.auto_set_font_size(False)
    the_table.set_fontsize(5)
    plt.title('Acc Diff in Percent for '+args.arch+' on '+args.dataset)
...
def main():
    if args.arch.startswith('vgg16'):
        ...
        layer_list = [10, 11, 12, 20, 21, 22, 23, 30, 31, 32, 33, 34, 35, 40, 41, 42]
    else:
        raise Exception('invalid args.arch')
    index = layer_list.index(args.start_layer)
    layer_list = layer_list[index:]

    # Data
    print('==> Preparing dataset %s' % args.dataset)
    num_classes, testloader, id2label = create_dataloader(args.dataset, args.dataset_path)

    # model list
    modelname_dict = dict()  # {11: 'vgg161x1lmp_1153_300', ...}
    modelname_list = os.listdir(args.resume)
    for name in modelname_list:
        tmp = name.split('_')
        assert len(tmp) == 3, "model names are in different format: '{}'".format(args.resume)
        modelname_dict[int(tmp[1][:2])] = name
    ...
    for layer in layer_list:
        ...
        # Load checkpoint.
        pp = os.path.join(args.resume, modelname_dict[layer], 'model_best.pth.tar')
        assert os.path.isfile(pp), 'Error: no checkpoint directory found!'
        checkpoint = torch.load(pp)
        model.load_state_dict(checkpoint['state_dict'])  # sanity check
        _, perclass_acc, test_acc = test(testloader, model, use_cuda)
        print(layer)
