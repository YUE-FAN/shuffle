...
def main():
    ...
    if args.evaluate:
        print('\nEvaluation only')
        test(testloader, model, criterion, start_epoch, use_cuda)
        return
...
def test(testloader, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        i = 78
        inputs = inputs[i, :, :, :].view(1, 3, 32, 32)
        # np.save('/nethome/yuefan/fanyue/dconv/img.npy', inputs.detach().cpu().numpy())
        targets = targets[i].view(1)
        print(inputs.size())
        print(inputs)
        print(CIFAR100_LABELS_LIST[targets])
        # compute output
        outputs = model(inputs)
