'''
Take the ref, train a VGG16_4353

The first 2 layers are frozen, lr is 10 times smaller than training from scratch,
last 4 conv layers and FCs are initialized as if it were from scratch.
...
from models.cifar import VGG16_1x1
from models.cifar.vggfy import CONV_1x1
...
def main():
    ...
    # create model
    model = VGG16_1x1(0, 1000, True, 99)
    print('loading pre-trained weights')
    pp = '/BS/yfan/work/trained-models/dconv/checkpoints/small-imagenet/vgg161x1_90_imgsize32_DA/vgg161x1_9953_90/checkpoint.pth.tar'
    checkpoint = torch.load(pp)
    model.load_state_dict(checkpoint['state_dict'], False)
    model.conv43 = nn.Sequential(CONV_1x1(512, 512, stride=1, padding=0, bias=True),
                                 nn.MaxPool2d(kernel_size=2, stride=2))
    model.conv51 = CONV_1x1(512, 512, stride=1, padding=0, bias=True)
    model.conv52 = CONV_1x1(512, 512, stride=1, padding=0, bias=True)
    model.conv53 = nn.Sequential(CONV_1x1(512, 512, stride=1, padding=0, bias=True),
                                 nn.MaxPool2d(kernel_size=2, stride=2))
    model.fc = nn.Sequential(nn.Linear(512, 4096),
                             nn.ReLU(True),
                             nn.Linear(4096, 4096),
                             nn.ReLU(True),
                             nn.Linear(4096, 1000))
    ...
    for name, module in model.named_children():
        if name in ['conv11', 'conv12']:
            for p in module.parameters():
                p.require_grads = False
        if name in ['conv43', 'conv51', 'conv52', 'conv53']:
    ...
    print('Total params: %.2fM' % (sum(p.numel() for p in filter(lambda p: p.requires_grad, model.parameters())) / 1000000.0))
